...
fn find_smallest(a: Int, b: Int, c: Int) -> Int {
  let smallest_one = int.min(a, b)
  int.min(smallest_one, c)
}

fn calc_wrapping_paper(box: Box) {
  case box {
    Box(l, w, h) -> {
      let top_bot = l * w
      let front_back = w * h
...
pub fn parse_line_wrapping(input: String) {
...
pub fn parse_line_ribbon(input: String) {
  let split_string = input |> string.split("x")
  case split_string {
    [l, w, h] -> {
      let assert Ok(length) = int.parse(l)
